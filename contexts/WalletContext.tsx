...
export function WalletProvider({ children }: { children: React.ReactNode }) {
  ...
  const initializeWallet = (address?: string) => {
    ...
    localStorage.setItem('rizz.money_wallet', JSON.stringify(wallet))
  }
  ...
  const updateBalance = (balance: number) => {
    ...
    if (state.wallet) {
      ...
      localStorage.setItem('rizz.money_wallet', JSON.stringify(updatedWallet))
    }
  }
  ...
  useEffect(() => {
    ...
    const savedWallet = localStorage.getItem('rizz.money_wallet')
    if (savedWallet) {
      try {
        ...
        localStorage.removeItem('rizz.money_wallet')
        dispatch({ type: 'SET_LOADING', payload: false })
      }
    } else {
      dispatch({ type: 'SET_LOADING', payload: false })
